...
// @namespace    https://demo.schul-cloud.org/
// @version      0.2
// @description  A Onboarding Game for the HPI SchulCloud
// @author       Dennis Doerk
// @match        https://demo.schul-cloud.org/*
// @grant        none
// ==/UserScript==
...
`;
    ...
    let deleteEditButtons = () => {
        ...
        if ($('h1#page-title').text().match("Nächste Schritte")) {
    ...
    let main = () => {
        ...
        if (gameStage === "1") {
            addOverlay();
            addCloudyDialog();
            addCloudyDialogText("Hi und willkommen in der HPI Schul-Cloud. 😊 Mein Name ist Cloudy - ich bin dein Kontakt für das HPI Schul-Cloud-System.\n \n Gemeinsam schreiben wir heute Geschichte. Der Schulalltag hat sich gravierend verändert und heute wird ein neuer Meilenstein gesetzt.");
            addCloudyDialogPrimaryButton('Weiter');
            $("input.btn.btn-primary").click(function () {
                addCloudyDialogText("Ich will dir die Funktionen der Cloud zeigen. Also lass uns erst einmal sehen, was du bis jetzt schon über Clouds weißt. \n \n Starten wir daher erstmal mit einem kleinen Quiz! 😊");
        ...
        if (gameStage == "2") {
            ...
            function populate() {
                if (quiz.isEnded()) {
                    ...
                    if (quiz.result !== undefined) {
                        ...
                        element.innerHTML = (quiz.result ? "<p style=\"color:green;\">Die Antwort '" + quiz.lastAnswer + "' war richtig!</p>" : "<p style=\"color:#b10438;\"> Die Antwort '" + quiz.lastCorrectAnswer + "' war falsch! Richtig wäre'" + quiz.lastAnswer + "'gewesen.</p>") + quiz.getQuestionIndex().text;
                    } else {
            ...
            function showScores() {
                ...
                if (quiz.score > 3) {
                    gameOverHTML += "<p>Wow, du weißt ja schon vieles über Clouds! 💪 Ich will dir jetzt eine ganz besondere Cloud vorstellen: die HPI Schul-Cloud!</p>"
                } else if (quiz.score > 1) {
                    gameOverHTML += "<p>Du scheinst ja schon einiges über Clouds zu wissen. 👍 Ich will dir jetzt eine ganz besondere Cloud vorstellen: die HPI Schul-Cloud!</p>"
                } else {
                    gameOverHTML += "<p>Clouds scheinen für dich noch neu zu sein. 😉 Ich will dir jetzt eine ganz besondere Cloud vorstellen: die HPI Schul-Cloud!</p>"
                }
            ...
            var questions = [
                ...
                new Question("Was ist eine Cloud?", ["Eine Bereitstellung von Speicherplatz, Software und Rechenleistungen über das Internet", "Wolkenförmige Zuckerwatte", "Sichere Messenger Alternative"], "Eine Bereitstellung von Speicherplatz, Software und Rechenleistungen über das Internet"),
                new Question("Was ist ein Vorteil einer Cloud?", ["Clouds bieten Datenzugriff von überall aus", "Durch Clouds hat man unbegrenzte mobile Daten", "Sie können nicht gehackt werden"], "Clouds bieten Datenzugriff von überall aus"),
                new Question("Was ist ein weiterer Vorteil einer Cloud?", ["Clouds sind unabhängig vom Internet", "Über Clouds ist eine Zusammenarbeit mit anderen möglich", "Stromsparende Alternative zu Servern"], "Über Clouds ist eine Zusammenarbeit mit anderen möglich"),
        ...
        if (gameStage == "3") {
            ...
            let videoFunction = () => {
                ...
                hackerWriting('Was wäre das Internet ohne Trolle?~Was wäre Schule ohne technische Pannen?~Diese Cloud-Tour ist vorbei, bevor sie angefangen hat!~ MUHAHAHAHAHAHAA! 👿');
        ...
        if (gameStage == "4") {
            addOverlay();
            addCloudyDialog();
            addCloudyDialogText("Oh Nein! 😱 \n Es wird versucht, ins System einzudringen! 😤 \n Kannst du mir helfen, den Hacker-Angriff abzuwehren?");
            addCloudyDialogSecondaryButton('Nein');
            addCloudyDialogPrimaryButton('Ja');
            $("input.btn.btn-primary").unbind("click").click(function () {
                addCloudyDialogText("Cool! Lass uns das System verteidigen! 😎");
                addCloudyDialogPrimaryButton('Na dann mal weiter!');
                $("input.btn.btn-primary").unbind("click").click(function () {
                    addCloudyDialogText("Ich habe Sorge, dass wir abgehört werden. Deswegen habe ich meine Nachricht an dich verschlüsselt. \n Um sie zu entschlüsseln, gehe in den Bereich “Dateien” > “meine persönlichen Dateien”. \n \n Übrigens: im unteren Bereich des Bildschirms könnt ihr immer die aktuellen Schritte nachverfolgen. 👇");
                    addInfoFooter("Aufgabe: Bereich Meine Dateien > Meine persönlichen Dateien > “Nachricht entschlüsseln” öffnen");
            ...
            $("input.btn.btn-secondary").click(function () {
                addCloudyDialogText("Ich brauche aber deine Hilfe. 😇 Komm, wir schaffen das! 😎")
                removeCloudyDialogSecondaryButton();
                addCloudyDialogPrimaryButton('Alles klar!');
            });
        }
        if (gameStage == "5") {
            addInfoFooter("Aufgabe: Bereich Dateien > meine persönlichen Dateien > Ordner “Nachricht entschlüsseln öffnen” > Bild öffnen > Nachricht entschlüsseln!");
        ...
        if (gameStage == "6") {
            if (location.pathname.match("/homework/")) {
                addOverlay();
                addCloudyDialog();
                addCloudyDialogText("Aloha! \n Cool, du hast meine Nachricht also entschlüsselt. 👏 \n Hier im Bereich “Aufgaben” habe ich alle Schritte zusammengefasst, die wichtig sind, um die Attacke abzuwenden! Unter dem Punkt ‘Nächste Schritte’ sage ich dir, wie wir weitermachen.");
                addCloudyDialogPrimaryButton('Ok!');
                $("input.btn.btn-primary").click(function () {
                    removeOverlay();
                    addInfoFooter("Gehe zu: ‘Nächste Schritte’");
                    nextGameStage(7);
                });
            }
        }
        if (gameStage == "7") {
            addInfoFooter("Gehe zu: ‘Nächste Schritte’");
            if (location.pathname.match("/homework/") && $('h1#page-title').text().match("Nächste Schritte")) {
                nextGameStage(8);
            }
        }
        if (gameStage == "8") {
            if (location.pathname.match("/news/")) {
                addOverlay();
                addCloudyDialog();
                addCloudyDialogText("Willkommen bei den Neuigkeiten!\nDer Hacker hat sich ein bisschen an den Überschriften ausgetobt. 😵\nDabei sollte dir auch eine Überschrift auffallen, die etwas mit dem Hackerangriff zu tun hat. Schaue im dazugehörigen Artikel nach weiteren Informationen und Anweisungen.");
        ...
        if (gameStage == "9") {
            addInfoFooter("Gehe zu: Neuigkeiten > öffne den Artikel über Hacker!");
            if (location.pathname.match("/news/") && $('h1#page-title').text() === "Wtllroeve Shsneiwei üreb Kheacr-Vhrlateen nfgdeune") {
                setTimeout(function () {
                    addOverlay();
                    addCloudyDialog();
                    addCloudyDialogText("Jackpot! \n Ich konnte Zugriff auf die Webcam des Hackers gewinnen. Diese sollten wir uns näher ansehen. \n \nGehe zum Bereich: Kurse > Wie man Hacker stoppt > Webcam Bilder.");
                    addCloudyDialogPrimaryButton('Ok!');
                    $("input.btn.btn-primary").click(function () {
                        removeOverlay();
                        nextGameStage(10);
        ...
        if (gameStage == "10") {
            addInfoFooter("Gehe zu: Kurse > Wie man Hacker stoppt > Webcam Bilder.");
            if (location.pathname.match("/topics/5db0b8eebe760b003157f1f9/")) {
                removeInfoFooter();
                addOverlay();
                addCloudyDialog();
                addCloudyDialogText("Schaue dir alle Bilder genau an. Wenn wir sein Master-Passwort herausfinden, können wir seinen Angriff abwehren.\n\nDas Masterpasswort ist der Lieblingsgegenstand des Hackers, welchen er immer bei sich trägt. 👀\n\nGib das Master-Passwort ein, um Zugriff auf ein spezielles Programm zu erhalten.");
                addCloudyDialogPrimaryButton('Ok!');
                $("input.btn.btn-primary").click(function () {
                    removeOverlay();
                    addInfoFooter("Gib das Master-Passwort hier ein:");
                    $('#infofootercontainer').append("<input type=\"text\" id=\"inputPassword\" class=\"form-control\" style=\"margin-bottom: 8px;\"> <input type=\"button\" class=\"btn btn-primary form-control\" value=\"Passwort prüfen\" id=\"checkButton\">");
                    $("input#checkButton").click(function () {
                        if ($('input#inputPassword').val().toLowerCase().match(/^(handy|smartphone|dergerät|telefon)$/)) {
                            removeInfoFooter();
                            addOverlay();
                            addCloudyDialog();
                            addCloudyDialogText("Yeah! Gut gemacht!\n\nDu hast ein Programm freigeschaltet, mit dem wir die geografische Position des Hackers bestimmen können! 📍\n\nGehe nun in das zweite Thema dieses Kurses: 'Finde die Position des Hackers'.\n");
                            addCloudyDialogPrimaryButton('Ok!');
                            $("input.btn.btn-primary").click(function () {
                                removeOverlay();
                                nextGameStage(12); // skip 11, this was not needed anymore
                            });
                        } else {
                            $('input#inputPassword').css("background-color", "rgba(255,0,0,0.2)")
                            addInfoFooter("Leider war das nicht das gesuchte Wort. Bitte finde das richtige Passwort und gib es hier ein:");
                        }
                    });
                });
            }
        ...
        if (gameStage == "12") {
            addInfoFooter("Gehe zu: Kurse > Wie man Hacker stoppt > Finde die Position des Hackers.");
            if (location.pathname.match("/topics/5db0b92d9f75de0031cc0509/")) {
                removeInfoFooter();
                addOverlay();
                addCloudyDialog();
                addCloudyDialogText("Okay, mit diesem Tool können wir den Hacker lokalisieren. 📍\n\nLies den Aufgabentext darunter und übernimm die Werte in das Programm, dann erhältst du einen Wert für X.\nMit diesem Wert können wir den Algorithmus lösen und seine Position bestimmen.");
                addCloudyDialogPrimaryButton('Ok!');
                $("input.btn.btn-primary").click(function () {
                    removeOverlay();
                    addInfoFooter("Kurs-Thema: Finde die Position des Hackers > Werte in Geogebra übernehmen > X-Wert hier eingeben:");
                    $('#infofootercontainer').append("<input type=\"text\" id=\"inputGeolocation\" class=\"form-control\" style=\"margin-bottom: 8px;\"> <input type=\"button\" class=\"btn btn-primary form-control\" value=\"Position prüfen!\" id=\"checkButton\">")
                    $("input#checkButton").click(function () {
                        if ($('input#inputGeolocation').val().toLowerCase().match(/^(2)$/)) {
                            removeInfoFooter();
                            addOverlay();
                            addCloudyDialog();
                            addCloudyDialogText("Ich habe die Daten ausgewertet und siehe da: der Angriff wurde 20 Meter von deiner aktuellen Position aus ausgeführt! 😲");
                            ...
                            addInfoFooter("Leider war das nicht der gesuchte X-Wert um die Position zu bestimmen. Bitte finde de richtigen X-Wert und gib ihn hier ein:");
                        }
                    });
                });
            }
        ...
        if (gameStage == "13") {
            ...
            $('#entercontainer').append('<img id="theImg" style=\"z-index: 12\"src="' + hackerImageData + '" />')
            $('#entercontainer').append('<div id=\"enterthecloudfooter\" style=\"position: absolute;background: black;bottom: 0;left: 0;right: 0;box-shadow: 0px 3px 15px lightgrey, 0 0 11px lightgreen;width: 95%;max-width: 700px;margin: auto;color: green;padding: 10px;z-index: 13; min-height: 150px;\"><p>&gt;_ Hacker Message:</p><p id=\"hacker_textchat\"></p></div>');
            hackerWriting('Ok, ok, ok. Ihr habt mich erwischt.~ Ich bin eine Schülerin aus der Klasse unter euch … Ich wollte die Cloud hacken, um alle eure Hausaufgaben und Referate abzugreifen, damit ich nächstes Jahr weniger Arbeit habe … 💩');
        }
        if (gameStage == "14") {
            addOverlay();
            $('#entercontainer').append('<canvas id=\"myCanvasMatrix\" style=\" position: absolute; width: 100%; height: 100%;\"></canvas>');
            addCloudyDialog();
            addCloudyDialogText("Ciao Kakao, Ende Gelände, Tschüssikowsky, tüdelü! \nDer Hacker ist dank euch offiziell von der Cloud verbannt! 💥");
            addCloudyDialogPrimaryButton('Super!');
            $("input.btn.btn-primary").click(function () {
                addCloudyDialogText("Du bist hammermäßig! Du hast es geschafft die Cloud zu retten und ganz nebenbei viele der Funktionen kennengelernt. 😉\n\n In diesem Spiel wird zwar ein Hacker-Angriff simuliert – aber eines der obersten Prinzipien der HPI-Schul-Cloud ist die digitale Sicherheit. So einfach, wie wir es hier dargestellt haben, kann man uns nicht hacken!");
                addCloudyDialogPrimaryButton('Weiter!');
                $("input.btn.btn-primary").unbind("click").click(function () {
                    addCloudyDialogText("Falls ihr mal Probleme mit der HPI Schul-Cloud haben solltet, macht nicht eure Lehrer dafür verantwortlich. Wir haben einen Hilfebereich und freuen uns sehr, wenn ihr uns Feedback gebt!");
                    addCloudyDialogPrimaryButton('Weiter!');
                    $("input.btn.btn-primary").unbind("click").click(function () {
